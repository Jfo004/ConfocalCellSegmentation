...
public class Stack_Focuser_
{
	...
	public void run(ImageProcessor ip)
	{
        ...
        /**
         * Setup of focuser and focuses image stack. use k value 11 for default
         * value. changes imagePlus, does not return a new flattened stack.
         * @param inImp ImagePlus to be flattened. performs 3d-gaussian
         * @param kValue value for kernel (n * n)
         */
        public void focusBF(ImagePlus inImp, int kValue) {
            //Setting variables 
            k_size = kValue;
             if (k_size < 3) {
                 k_size = 3;
             ...
             create_map = false;
             onefocus = false;
            GaussianBlur3D.blur(inImp, 0.8, 0.8, 0.8);
            i_stack = inImp.getStack();
            o_width = inImp.getWidth();
            o_height = inImp.getHeight();
            o_dim = o_width * o_height;
            n_slices = inImp.getStackSize();
            r = new Rectangle(0, 0, o_width, o_height);
            i_stack.setRoi(r);
            n_width = r.width;
            n_height = r.height;
            ...
            Calibration cal = inImp.getCalibration().copy();
            String title = inImp.getTitle();
             
            //Checking imageprocessor
            ImageProcessor ip_p = inImp.getProcessor();
            if (ip_p instanceof ByteProcessor)
                focused_ip = focusGreyStack(i_stack, BYTE);
            else if (ip_p instanceof ShortProcessor)
                focused_ip = focusGreyStack(i_stack, SHORT);
            else if (ip_p instanceof FloatProcessor)
                focused_ip = focusGreyStack(i_stack, FLOAT);
            inImp.flush();
            inImp.setProcessor(title, focused_ip);
            inImp.setCalibration(cal);            
        }
                //Run focuser, replace imp 
